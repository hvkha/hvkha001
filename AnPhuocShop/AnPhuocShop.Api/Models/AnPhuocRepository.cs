...
namespace AnPhuocShop.Api.Models
{
    public class AnPhuocRepository: IAnPhuocRepository
    {
        ...
        public async Task<AnPhuoc> UpdateAnPhuoc(AnPhuoc anphuoc)//cập nhật hoặc sữa sản phẩm
        {
            var result = await appDbContext.AnPhuocs
                .FirstOrDefaultAsync(a => a.AnPhuocId == anphuoc.AnPhuocId);//Hàm tìm kiếm sản phẩm
            if (result != null)
            {
